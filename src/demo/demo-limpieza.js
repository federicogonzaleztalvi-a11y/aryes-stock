...
export const demoLimpieza = {
  ...
  get cfes() {
    ...
    (this.ventas || []).forEach((v, i) => {
      ...
      cfes.push({
        id: 'cfe-limpieza-'+i, numero: 'E-'+String(1000+i).padStart(4,'0'),
    ...
    for (let m=5; m>=1; m--) {
      ...
      for (let j=0; j<(8+Math.floor(m*1.5)); j++) {
        ...
        cfes.push({
          id: 'cfe-limpieza-h'+m+'-'+j, numero: 'E-'+String(800+m*20+j).padStart(4,'0'),
          tipo: 'e-Factura', moneda: 'UYU',
  ...
  get cobros() {
    ...
    (this.ventas || []).forEach((v, i) => {
      if ((v.pago==='efectivo'||v.pago==='transferencia') && v.estado==='entregada') {
        ...
        const total = v.items.reduce((s,it)=>s+it.qty*it.price,0);
        cobros.push({
          id: 'cob-limpieza-'+i, cliente_id: v.cliente_id,
          monto: total, metodo: v.pago, fecha: d.toISOString().split('T')[0],
          notas: '', facturas_aplicar: ['cfe-limpieza-'+i],
    ...
    for (let m=5; m>=1; m--) {
      ...
      for (let j=0; j<6; j++) {
        ...
        const id2 = new Date(md.getFullYear(), md.getMonth(), day);
        cobros.push({
          id: 'cob-limpieza-h'+m+'-'+j, cliente_id: this.clients[j%this.clients.length]?.id||'c1',
