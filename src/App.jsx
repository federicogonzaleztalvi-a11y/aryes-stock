...
function AryesApp(){
  ...
  if(!user){
    const USERS=[{username:'admin',password:'aryes2024',role:'admin',name:'Administrador'},{username:'operador',password:'stock123',role:'operador',name:'Operador'},{username:'vendedor',password:'ventas123',role:'vendedor',name:'Vendedor'}];
    const doLogin=(u,p)=>{
    ...
    return(
      <div style={{minHeight:'100vh',background:'#f8f9fa',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'Georgia,serif'}}>
        <div style={{background:'#fff',borderRadius:16,padding:'48px 40px',boxShadow:'0 4px 24px rgba(0,0,0,.08)',width:360,textAlign:'center'}}>
          <img src="/aryes-logo.png" alt="Aryes" style={{height:72,marginBottom:8,objectFit:'contain'}} />
          <h1 style={{fontFamily:'Playfair Display,serif',fontSize:28,color:'#1a1a1a',margin:'0 0 4px'}}>Aryes Stock</h1>
          <p style={{color:'#888',fontSize:13,margin:'0 0 32px'}}>Sistema de gestion de inventario</p>
          <input id="lu" type="text" placeholder="Usuario" style={{width:'100%',padding:'11px 14px',border:'1px solid #e5e7eb',borderRadius:8,fontSize:14,fontFamily:'inherit',boxSizing:'border-box',marginBottom:12}} onChange={e=>lu=e.target.value} />
          <input id="lp" type="password" placeholder="Contrasena" style={{width:'100%',padding:'11px 14px',border:'1px solid #e5e7eb',borderRadius:8,fontSize:14,fontFamily:'inherit',boxSizing:'border-box',marginBottom:20}} onChange={e=>lp=e.target.value} onKeyDown={e=>e.key==='Enter'&&doLogin(lu,lp)} />
          <button onClick={()=>doLogin(lu,lp)} style={{width:'100%',padding:'12px',background:G,color:'#fff',border:'none',borderRadius:8,fontSize:15,fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>Ingresar</button>
        </div>
      </div>
    );
  }
  ...
  const logout=()=>{LS.set('aryes-session',null);setUser(null);};

  return(
    <div style={{display:'flex',minHeight:'100vh',background:'#f8f9fa',fontFamily:'Georgia,serif'}}>
      {/* SIDEBAR */}
      <aside style={{width:200,background:'#1a1a1a',display:'flex',flexDirection:'column',flexShrink:0,overflowY:'auto'}}>
        <div style={{padding:'16px 12px 8px',borderBottom:'1px solid #333'}}>
          <img src="/aryes-logo.png" alt="Aryes" style={{height:44,objectFit:'contain',filter:'brightness(0) invert(1)',display:'block',margin:'0 auto'}} />
        </div>
        <nav style={{flex:1,padding:'8px 0'}}>
          {NAV.map(n=>(
            <button key={n.id} onClick={()=>setTab(n.id)} style={{width:'100%',display:'flex',alignItems:'center',gap:8,padding:'10px 14px',background:tab===n.id?G:'transparent',color:tab===n.id?'#fff':'#ccc',border:'none',cursor:'pointer',fontSize:12,fontFamily:'inherit',textAlign:'left',transition:'all .15s'}}>
        ...
        <div style={{padding:'12px',borderTop:'1px solid #333'}}>
          <div style={{fontSize:11,color:'#888',marginBottom:6}}>{user.name} ({user.role})</div>
          <button onClick={logout} style={{width:'100%',padding:'7px',background:'#333',color:'#ccc',border:'none',borderRadius:6,cursor:'pointer',fontSize:11,fontFamily:'inherit'}}>Cerrar sesion</button>
        </div>
      </aside>
      {/* MAIN */}
