  ...
  return(
    <>
      ...
      {/* →→ COMMAND PALETTE →→ */}
        ...
        {ConfirmDialog}
        {/* →→ MODALS →→ */}
      {modal?.type==="product"&&<Modal title={editProd?"Editar producto":"Nuevo producto"} sub="Inventario" onClose={()=>{setModal(null);setEditProd(null);}}><ProductForm product={editProd} suppliers={suppliers} brandCfg={brandCfg} categories={[...new Set((products||[]).map(p=>p.category||p.categoria).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'es'))} onSave={saveProduct} onClose={()=>{setModal(null);setEditProd(null);}}/>{editProd&&<div style={{padding:'0 16px 16px'}}><button onClick={()=>{setModal(null);setEtiquetaProd(editProd);}} style={{width:'100%',background:'#f5f5f7',border:'1px solid #e0e0dc',borderRadius:8,padding:'10px',fontSize:13,cursor:'pointer',color:'#4a4a48'}}>🏷️ Imprimir etiqueta de producto</button></div>}</Modal>}
