    ...
    <section style={{padding:"28px 36px",maxWidth:700,margin:"0 auto"}}>
      ...
      <div style={{background:"#fff",borderRadius:12,padding:20,boxShadow:"0 1px 4px rgba(0,0,0,.06)",marginBottom:16}}>
        ...
        {(sel.items||[]).map(it=>{
          ...
          return(
            <div key={key} onClick={()=>toggleValidar(key)} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",borderRadius:8,marginBottom:8,cursor:"pointer",background:v?.validado?"#f0fdf4":"#f9fafb",border:"2px solid "+(v?.validado?"#059669":"#e5e7eb"),transition:"all .15s"}}>
              <div style={{width:24,height:24,borderRadius:"50%",background:v?.validado?"#059669":"#e5e7eb",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:14,flexShrink:0}}>{v?.validado?"✓":""}</div>
              <div style={{flex:1}}>
                <div style={{fontSize:14,fontWeight:600,color:"#1a1a1a"}}>{it.nombre}</div>
                <div style={{fontSize:12,color:"#888"}}>{it.cantidad} {it.unidad}</div>
              </div>
              <span style={{fontSize:12,fontWeight:700,color:v?.validado?"#059669":"#9ca3af"}}>{v?.validado?"VALIDADO":"Tocar para validar"}</span>
            </div>
          );
        })}
      ...
      <div style={{background:todosValidados?"#f0fdf4":"#f9fafb",border:"2px solid "+(todosValidados?"#059669":"#e5e7eb"),borderRadius:10,padding:"12px 16px",marginBottom:16,fontSize:13,color:todosValidados?"#059669":"#9ca3af",fontWeight:600,textAlign:"center"}}>
      ...
      <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
        ...
        <button onClick={confirmarPacking} disabled={!todosValidados} style={{padding:"10px 28px",background:todosValidados?"#059669":"#d1d5db",color:"#fff",border:"none",borderRadius:8,cursor:todosValidados?"pointer":"not-allowed",fontWeight:700,fontSize:14}}>Confirmar packing</button>
      </div>
    </section>
  );
