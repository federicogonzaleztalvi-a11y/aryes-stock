...
const ClientsTab=({products,session})=>{
  ...
  if(detail) return(
    <div style={{padding:'32px 40px',maxWidth:800}}>
      <button onClick={()=>setDetail(null)} style={{background:'none',border:'none',fontSize:13,color:'#059669',cursor:'pointer',fontFamily:'inherit',marginBottom:20,display:'flex',alignItems:'center',gap:6}}>← Volver a clientes</button>
  ...
  return(
    <div style={{padding:'32px 40px',maxWidth:900}}>
      <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:24}}>
        ...
        {canEdit&&<button onClick={startNew} style={{padding:'9px 18px',background:'#059669',color:'#fff',border:'none',borderRadius:8,fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:'inherit'}}>+ Nuevo cliente</button>}
      </div>
      ...
      <div style={{display:'flex',gap:12,marginBottom:20,flexWrap:'wrap'}}>
        ...
        <div style={{background:'#f0f7ec',border:'1px solid #b8d9a8',borderRadius:10,padding:'12px 18px',minWidth:120,flex:1}}>
          ...
          <div style={{fontSize:20,fontWeight:700,color:'#059669'}}>{clients.length}</div>
                ...
                {products.sort((a,b)=>a.name.localeCompare(b.name)).map(p=>(
                  <label key={p.id} style={{display:'flex',alignItems:'center',gap:8,padding:'6px 8px',cursor:'pointer',borderRadius:6,fontSize:13,':hover':{background:'#f0f0ec'}}}>
                    <input type="checkbox" checked={(form.products||[]).includes(p.id)}
                      onChange={()=>toggleProduct(p.id)} style={{accentColor:'#059669'}}/>
        ...
        :<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:12}}>
          {clients.map(c=>(
            <div key={c.id} style={{background:'#fff',border:'1px solid #e2e2de',borderRadius:12,padding:20,cursor:'pointer',transition:'box-shadow .15s'}}
              ...
              {c.products?.length>0&&<div style={{fontSize:11,color:'#059669',marginTop:8,fontWeight:600}}>{c.products.length} productos asignados</div>}
