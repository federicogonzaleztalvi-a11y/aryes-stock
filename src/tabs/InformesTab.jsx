...
function InformesTab(){
  ...
  const exportStock=()=>{const h=["Producto","Stock","Unidad","Precio","Stock Min","Proveedor","Estado"];const rows=prods.map(p=>{const min=Number(p.minStock??p.rop??5);return[p.nombre||p.name||"",p.stock||0,p.unidad||p.unit||"u",p.precioVenta||p.precio||p.price||0,min,p.brand||p.proveedor||"",Number(p.stock||0)===0?"Sin stock":Number(p.stock||0)<=min?"Critico":"OK"];});downloadCSV(toCSV(h,rows),"stock-"+hoy.toISOString().split("T")[0]+".csv");};
  ...
  const imprimirRemito=(venta)=>{
    // Escapar TODO valor de usuario antes de inyectarlo en el HTML del remito.
    // Sin esto, un nombre de cliente/producto con "<script>" ejecutaría XSS en
    // la ventana de impresión (document.write).
    const esc=(s)=>String(s==null?"":s).replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));
    const emp=esc(LS.get("aryes-empresa", "Stock"));
    const its=venta.items||[];
    const rows=its.map(it=>"<tr><td>"+esc(it.nombre)+"</td><td>"+esc(it.cantidad)+" "+esc(it.unidad)+"</td><td>$"+Number(it.precioUnit).toLocaleString("es")+"</td><td>$"+Number(it.cantidad*it.precioUnit).toLocaleString("es")+"</td></tr>").join("");
    const desc=Number(venta.descuento)>0?"<p style=\"text-align:right;color:#92400e\">Descuento "+esc(venta.descuento)+"%</p>":"";
    const nro=esc(venta.nroVenta);
  ...
  const BTN=({label,icon,onClick})=>(
    <button onClick={onClick} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 16px",background:"#fff",border:"2px solid #059669",borderRadius:10,cursor:"pointer",fontFamily:"inherit",width:"100%",textAlign:"left",marginBottom:8}}>
    ...
    <section style={{padding:"28px 36px",maxWidth:1100,margin:"0 auto"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24,flexWrap:"wrap",gap:12}}>
        ...
        <div style={{display:"flex",gap:6}}>{["semana","mes","trimestre"].map(p=>(
          <button key={p} onClick={()=>setPeriodo(p)} style={{padding:"6px 14px",borderRadius:20,border:"2px solid "+(periodo===p?"#059669":"#e5e7eb"),background:periodo===p?"#059669":"#fff",color:periodo===p?"#fff":"#666",fontWeight:600,fontSize:12,cursor:"pointer"}}>
            {p==="semana"?"7 dias":p==="mes"?"30 dias":"90 dias"}</button>))}</div></div>
      {msg&&<div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,padding:"10px 16px",marginBottom:16,color:"#059669",fontSize:13,fontWeight:600}}>{msg}</div>}
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:28}}>
        <div style={{background:"#fff",borderRadius:10,padding:"16px 18px",boxShadow:"0 1px 4px rgba(0,0,0,.06)",border:"2px solid "+(stockCritico>0?"#ef4444":"transparent")}}>
          ...
          <div style={{fontSize:26,fontWeight:800,color:stockCritico>0?"#ef4444":"#059669"}}>{prods.length}</div>
        ...
        <div style={{background:"#fff",borderRadius:10,padding:"16px 18px",boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
          ...
          <div style={{fontSize:26,fontWeight:800,color:"#059669"}}>{ventasP.length}</div>
          <div style={{fontSize:11,color:"#888",marginTop:3}}>${totalVentasMes.toLocaleString("es")} facturado</div></div>
        ...
        <div style={{background:"#fff",borderRadius:10,padding:"16px 18px",boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
          ...
          <div style={{fontSize:26,fontWeight:800,color:"#059669"}}>{entregasOk}/{entregasTotal}</div>
          ...
          <div style={{background:"#fff",borderRadius:10,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
            {ventas.slice(0,12).map((v,i)=>(
              <div key={v.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderBottom:"1px solid #f3f4f6",background:i%2===0?"#fff":"#fafafa"}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:13,fontWeight:700,color:"#059669"}}>{v.nroVenta}</div>
                  <div style={{fontSize:11,color:"#666"}}>{v.clienteNombre} · {v.fecha}</div>
                </div>
                <div style={{fontWeight:700,fontSize:13}}>${Number(v.total||0).toLocaleString("es")}</div>
                <button onClick={()=>imprimirRemito(v)} style={{padding:"5px 12px",background:"#059669",color:"#fff",border:"none",borderRadius:6,cursor:"pointer",fontSize:11,fontWeight:700}}>
                  Imprimir</button>
              </div>))}
