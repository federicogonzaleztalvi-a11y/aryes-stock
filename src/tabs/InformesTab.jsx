...
function InformesTab(){
  ...
  const imprimirRemito=(venta)=>{
    ...
    const rows=its.map(it=>"<tr><td>"+it.nombre+"</td><td>"+it.cantidad+" "+it.unidad+"</td><td>$"+Number(it.precioUnit).toLocaleString("es")+"</td><td>$"+Number(it.cantidad*it.precioUnit).toLocaleString("es")+"</td></tr>").join("");
    const desc=Number(venta.descuento)>0?"<p style=\"text-align:right;color:#92400e\">Descuento "+venta.descuento+"%</p>":"";
    const html="<!DOCTYPE html><html><head><meta charset='UTF-8'><title>Remito "+venta.nroVenta+"</title><style>body{font-family:Arial,sans-serif;margin:40px;color:#1a1a1a;}.hdr{display:flex;justify-content:space-between;border-bottom:2px solid #059669;padding-bottom:16px;margin-bottom:24px;}.emp{font-size:22px;font-weight:700;color:#059669;}table{width:100%;border-collapse:collapse;margin:16px 0;}th{background:#059669;color:#fff;padding:9px;text-align:left;font-size:13px;}td{padding:8px 9px;border-bottom:1px solid #eee;font-size:13px;}.tot{text-align:right;font-size:18px;font-weight:700;color:#059669;margin-top:8px;}.ftr{margin-top:40px;border-top:1px solid #eee;padding-top:12px;font-size:11px;color:#aaa;text-align:center;}</style></head><body><div class='hdr'><div><div class='emp'>"+emp+"</div></div><div style='text-align:right'><b>REMITO "+venta.nroVenta+"</b><br>"+venta.fecha+"<br><span style='font-size:11px;padding:2px 8px;background:#f0fdf4;border-radius:4px;color:#059669;font-weight:700'>"+(venta.estado||"").toUpperCase()+"</span></div></div><p><b>Cliente:</b> "+venta.clienteNombre+"</p><table><thead><tr><th>Producto</th><th>Cant.</th><th>Precio</th><th>Subtotal</th></tr></thead><tbody>"+rows+"</tbody></table>"+desc+"<div class='tot'>TOTAL: $"+Number(venta.total||0).toLocaleString("es")+"</div><div class='ftr'>"+emp+" · "+new Date().toLocaleDateString("es")+"</div><script>window.onload=function(){window.print();}<" + "/script></body></html>";
