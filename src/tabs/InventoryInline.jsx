                    ...
                    <tr key={p.id} style={{borderBottom:`1px solid ${T.border}`,background:i%2===0?T.card:T.cardWarm,transition:"background .1s"}}
                      ...
                      <td style={{padding:"11px 13px"}}>
                        <div style={{display:"flex",gap:6}}>
                          <Btn small variant="ghost" onClick={()=>{{const found=products.find(x=>x.id===p.id);if(found)setEditProd({name:'',barcode:'',supplierId:'',unit:'kg',stock:0,unitCost:0,precioVenta:0,iva_rate:(brandCfg?.iva_default||22),imagen_url:'',descripcion:'',codigo:'',history:[],...found});setModal({type:"product"});}}}>Editar</Btn>
