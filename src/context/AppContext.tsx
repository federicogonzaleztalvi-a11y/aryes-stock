...
export function AppProvider({ session, onLogout, onSessionUpdate, children, demoState }: {
  ...
  useEffect(() => {
    ...
    const dp = demoState.products.map(p => ({ id:p.sku||p.id, uuid:p.id, nombre:p.name, marca:p.category, unidad:p.unit||'un', stock:p.stock, minStock:p.min_stock, precio:p.cost, precioVenta:p.price, unitCost:p.cost, supplierId:p.supplier_id, imagenUrl:p.imagen_url, descripcion:'', ivaRate:p.iva_rate||22, org_id:'demo', dailyUsage: Math.max(1, Math.round(p.stock * 0.08)) }));
